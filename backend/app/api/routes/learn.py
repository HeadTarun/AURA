...
from app.services.teaching_engine import generate_lesson, normalize_level
...
def debug_session(student_id: str) -> dict[str, Any]:
...
def learn(req: LearnRequest) -> dict[str, Any]:
    ...
    result = generate_lesson(req.topic, level, "\n\n".join(chunks))

    session["current_topic"] = req.topic.strip()
    session["level"] = level
