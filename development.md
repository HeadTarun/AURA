...
For Docker Compose, the backend is exposed at `http://localhost:8001` and the frontend is exposed at `http://localhost:3000` by default. You can change those host ports in `.env` with `BACKEND_PORT` and `FRONTEND_PORT`.
